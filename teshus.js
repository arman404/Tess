...
//  KONFIGURASI
// ============================================================
const BOT_TOKEN = process.env.BOT_TOKEN || '8754070391:AAEyTToblhJfTTnGxSF-aMbb2iI58Htbjm4';
const EMAIL_USER = process.env.EMAIL_USER || 'tesspambot@gmail.com';
const EMAIL_PASS = process.env.EMAIL_PASS || 'Max123##';
const ADMIN_ID = parseInt(process.env.ADMIN_ID) || 1574411746;
...
//  KEYBOARD & MENU
// ============================================================
function mainMenu(isPremium = false) {
    const buttons = [
        [Markup.button.callback('📝 Lapor Scam', 'report')],
        [Markup.button.callback('📧 Kelola Email', 'manage_emails')],
        [Markup.button.callback('ℹ️ Bantuan', 'help'), Markup.button.callback('📊 Status', 'status')]
    ];

    if (isPremium) {
        buttons.splice(1, 0, [Markup.button.callback('🏷️ Tag Scam (Premium)', 'scam_tag')]);
        buttons.push([Markup.button.callback('⭐ Ganti Email', 'change_email')]);
    } else {
        buttons.push([Markup.button.callback('⭐ Upgrade Premium', 'premium')]);
    }
